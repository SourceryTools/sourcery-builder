...
class BuildTaskTestCase(unittest.TestCase):
    ...
    def test_makefile_text(self):
        """Test makefile_text."""
        # This tests text from commands appears in the generated
        # makefiles, but the exact wrapper commands and their
        # execution are left to tests of BuildContext to validate.
        top_task = BuildTask(self.relcfg, None, '', True)
        sub1_task = BuildTask(self.relcfg, top_task, 'a', False)
        sub1_task.add_command(['test', 'cmd'])
        sub1_task.add_command(['test', '$'])
        sub2_task = BuildTask(self.relcfg, top_task, 'b', False)
        sub2_task.add_command(['test', 'cmd3'], cwd='/some/where')
        ...
        sub3_task.depend('/b')
        expect_deps = {}
        top_task.record_deps(expect_deps)
        text = top_task.makefile_text(self.build_context)
        deps, commands = parse_makefile(text)
        for key in expect_deps:
            for dep in expect_deps[key]:
                self.assertIn(dep, deps[key])
        self.assertIn('task-end', deps['all'])
        cmds_a = commands['task-end/a']
        cmds_b = commands['task-end/b']
        cmds_c = commands['task-end/c']
        self.assertIn('start-task', cmds_a[0])
        self.assertIn('test cmd', cmds_a[1])
        self.assertIn("test '$$'", cmds_a[2])
        self.assertIn('end-task', cmds_a[3])
        self.assertIn('start-task', cmds_b[0])
        self.assertIn('/some/where', cmds_b[1])
        self.assertIn('test cmd3', cmds_b[1])
        self.assertIn('/make/where', cmds_b[2])
        self.assertIn('$(MAKE) test arg', cmds_b[2])
        self.assertIn('rm -rf /empty/dir', cmds_b[3])
    ...
    def test_makefile_text_errors(self):
        """Test errors from makefile_text."""
        top_task = BuildTask(self.relcfg, None, '', True)
        sub1_task = BuildTask(self.relcfg, top_task, 'a', True)
        self.assertRaisesRegex(ScriptError,
                               'makefile_text called for non-top-level task '
                               '/a',
                               sub1_task.makefile_text, self.build_context)
